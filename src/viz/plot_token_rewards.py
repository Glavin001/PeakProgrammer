...
def plot_token_rewards(tokens: List[str], rewards: Union[float, List[float]], title='Rewards per Token'):

    if isinstance(rewards, (int, float)):
        rewards_tmp = [0] * len(tokens)
        rewards_tmp[-1] = rewards
        rewards = rewards_tmp

    # assert len(rewards) == len(tokens), "Rewards and tokens must have the same length"
    # include the lengths of each in error message
